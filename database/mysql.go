...
func (db MySQL) Validate() error {
  ...
  return errors.New("MySQL required `name` and `args`")
}

func (db MySQL) Perform(dir string) error {
  if err := db.Validate(); err != nil {
    return err
  }

  file := "MySQL_" + db.Name + ".sql"
  path := filepath.Join(dir, file)

  dump := exec.Command("mysqldump", "--result-file="+path)
